...
const Indicadores = ({ indicador }) => {
    ...
    useEffect(() => {
        ...
        let recibeRespuesta = null;

        (async function () {
            //ESTAMOS CONSULTANDO TODOS LOS INDICADORES Y VARIABLES DISPONIBLES PARA ESTE USUARIO, ACA NO ME INTERESA FILTRAR AUN
            ...
            recibeRespuesta = await respuesta;

            console.log("Recibe respuesta", recibeRespuesta);

            setDatos(respuesta);

            //ACA TENGO QUE INICIARLIZAR A "datosFiltrados": Porque en un principio en la ecuacion ya habran algunas variables
            /*creo que surge la necesidad de cuando se edite un indicador, poder tener en el JSON la familia raiz, para inicializar "datosFiltrados"*/
            /* console.log("recibeRespuesta", recibeRespuesta) */
            let raiz = recibeRespuesta.filter(operable => operable.variable_id === indicador.id)[0];


            //LUEGO INICIALIZAR "elementosOperacion" PARA QUE SE MUESTRE LA OPERACION PREDEFINIDA DEL INDICADOR A EDITAR
            setElementosOperacion(indicador.operation);

            //INICIALIZAR LAS "variablesUsadas" CONFORME A LAS USADAS EN EL INDICADOR A EDITAR
            setVariablesUsadas(indicador.operation.filter((elemento) => elemento.type === "Variable"));

            //POR ULTIMO (PENSARLO BIEN) ADJUNTAR LA RAIZ PERMITIDA, PARA LAS OPERACIONES
            setRaizPermitida(raiz.root_id);

            setDatosFiltrados(recibeRespuesta.filter(operable => operable.root_id === raiz.root_id))
        })()
                        ...
                        <section className={style.creacion__indicadores}>
                            ...
                            <div className={style.añade__indicadores}>
                                <form className={style.formulario}>
                                    ...
                                    <input name="indicator_name" className={style.formulario__input} type="text" defaultValue={indicador.name} onChange={(e) => { onlyOneSpace(e); setDatosCapturados({ ...datosCapturados, [e.target.name]: e.target.value }) }}></input>
                                    <label className={style.formulario__label}>Descripción</label>
                                    <textarea name="indicator_description" className={style.formulario__textarea} defaultValue={indicador.description} onChange={(e) => { onlyOneSpace(e); setDatosCapturados({ ...datosCapturados, [e.target.name]: e.target.value }) }} maxLength={250}></textarea>
                                    <div>
                                        <button className={style.añade__variable} type="button" onClick={() => vistaPermisos(datosCapturados, setDatosCapturados)}>Mis Indicadores</button>
                                    </div>
                                </form>
                            </div>
                            ...
                            <div className={style.opera__indicadores}>
                                ...
                                <section className={style.contenedor__operaciones}>
                                    {/*ESTE ES EL TABLERO DE OPERACIONES*/}
                                        ...
                                        return (
                                            <div className={style.card__variable} key={uuidv4()}>
                                                ...
                                                <div key={idx} className={style.card__grid}>
                                                    ...
                                                    <button datos={creaAtributo(operable.type_id, operable.variable_name, operable.type, operable.root_id)} className={style.añade__variable} name="variable" onClick={añadeOperacion} data-tip={`Al añadir este elemento, se filtrara la lista de elementos,<br/>solo se pueden operar entre compatibles`}>+</button>
                                                </div>
                                            </div>
                                        )
                                    })
